...
① 分析大纲 — 分析主题，生成调研框架和搜索计划
         ↓
② 采集数据 — ╭─ 在线模式：SearXNG / Exa 级联搜索 → Scrapling 批量抓取 → 数据池
              ╰─ 离线模式：直接读取本地文件（PDF/DOCX/TXT/MD）→ 数据池
         ↓
③ 依次撰写 — 逐章同步撰写，事实直接嵌入 prompt，不做工具调用
         ↓
...
4. 将多 agent 链式架构（大纲 → 数据采集 → 依次撰写 → 装配QA）翻译为当前工具的等价实现，保留各阶段的串行依赖关系
...
③ 依次撰写 — 逐章同步撰写，事实直接嵌入 prompt，不做额外工具调用
