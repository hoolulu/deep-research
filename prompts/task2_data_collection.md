...
**Layer 1 — 大纲建议源（最高优先级）**：读取 {TMPDIR}/outline.json 的 `source_suggestions` 数组（如 `["noaa.gov", "ipcc.ch"]`），对每个域名，用 SearXNG 搜索 `site:{域名} {子问题关键词}`。这些是大纲阶段根据主题定向推荐的源，最贴合主题。

**Layer 2 — SearXNG 主力搜索**：SearXNG 全网搜索，70+ 搜索引擎（含 Google/Bing/Brave/百度）并行。这是最广覆盖的搜索层，与 Layer 1 并行发出。
...
Step 1 — 搜索引擎健康检测
   ...
   **SearXNG 检测**（通过 dr_tools.py）：
   ...
   ☐ 输出 `{"engine": "none", "available": false}` → `searxng_available=false` → 标记 `all_search_unavailable=true`

Step 2 — 多源并行搜索（Layer 1-3 同时发出）

四层搜索中的 Layer 1-3 在本步骤中一次性全部发出，不串行等待：

**Layer 1 — 大纲建议源定向搜索**：如果 outline.json 包含 `source_suggestions`，对每个域名用 SearXNG 搜索 `site:{域名} {子问题关键词}`，一次性并行发出。

**Layer 2 — SearXNG 主力搜索**：
...
Step 3 — 免费数据源补强（Layer 4，与 Scrapling 抓取并行）
    ...
    **触发补强的任意条件**：
    1. `all_search_unavailable=true` → 强制执行（搜索引擎不可用）
...
## 硬规则
1. **搜索引擎策略**：SearXNG（Layer 1）全区搜索 + sources.json 优质源定向搜索（Layer 2）并行，搜索结果合并去重。搜索结果质量不足时（URL < 3 / 年份过旧 / 来源过少）触发免费源补强（Layer 3 兜底）。
...
## 作业
1. 完成 SearXNG + sources.json 优质源 + Scrapling/webfetch 数据收集 + 数据池提取
...
- 根据 `searxng_available` 及是否触发补强推导结构化字段：
  - `engines`（数组，列出实际可用的引擎名）：`["searxng"]` / `[]`
  - `free_fallback`（boolean）：是否触发了 Step 3 免费源补强
  - `english_fallback`（boolean）：是否执行了 Step 2b 英文补搜
  - `search_engine`（字符串，向下兼容）：沿用原规则
- **`unique_domains`**：从 data-pool.json 所有来源 URL 中提取域名（`urllib.parse.urlparse`），去重计数
```json
{"task":2,"source_count":14,"fact_count":41,"search_engine":"SearXNG","fetch_method":"🔧 Scrapling","data_pool_path":"{TMPDIR}/data-pool.json","cautions_path":"{TMPDIR}/cautions.json","data_limited":false,"searxng_available":true,"exa_available":false,"engines":["searxng"],"free_fallback":false,"english_fallback":false,"unique_domains":11}
```
